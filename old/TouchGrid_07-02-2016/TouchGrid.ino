...
void setup()
{
  ...
  // Create buttons for the summary
  menuB = new block*[2];
  for(int i=0; i<2;i++){
      menuB[i] = new block(myGLCD);
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    menuB[i]->setCoordinates(xcorL, ycorL, xcorR, ycorR);
    menuB[i]->setActiveColor(RGB_BLUE);
    menuB[i]->setPassiveColor(RGB_GRAY);
  }
  drawMenu();
  drawScreen(CS);
}
